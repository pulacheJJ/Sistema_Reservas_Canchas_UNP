...
<body x-data="{ sidebarOpen: false }" @keydown.escape.window="sidebarOpen = false" class="bg-slate-50 font-sans flex h-screen overflow-hidden selection:bg-blue-100 selection:text-blue-900">
    ...
    <main class="min-w-0 flex-1 flex flex-col overflow-hidden relative">
        
        <!-- Header Moderno -->
        <header class="bg-white/90 backdrop-blur-md shadow-sm border-b border-slate-200 px-3 sm:px-6 py-3 sm:py-4 flex justify-between items-center gap-2 z-20 sticky top-0">
            <div class="min-w-0 flex items-center gap-2 sm:gap-4">
                ...
                <h2 class="truncate text-base sm:text-xl font-bold text-slate-800 tracking-tight"><span class="sm:hidden">Reservas UNP</span><span class="hidden sm:inline">Plataforma de Reservas UNP</span></h2>
            </div>
            
            <div class="flex flex-shrink-0 items-center gap-2 sm:gap-6">
                <!-- Estado Sede -->
                ...
                <div x-data="{ open: false }" class="relative">
                    <button @click="open = !open" @click.away="open = false" class="flex items-center gap-1 sm:gap-3 hover:bg-slate-50 p-1 rounded-xl transition-colors" aria-label="Abrir menú de usuario">
                        <div class="w-9 h-9 sm:w-10 sm:h-10 bg-gradient-to-br from-blue-600 to-blue-800 rounded-full flex items-center justify-center text-white font-bold shadow-sm">
                    ...
                    <!-- Dropdown Content -->
                    <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute right-0 mt-2 w-52 max-w-[calc(100vw-1.5rem)] bg-white rounded-xl shadow-lg py-1 border border-slate-100 z-50" style="display: none;">
        ...
        <div class="min-w-0 flex-1 overflow-y-auto overflow-x-hidden p-3 sm:p-4 md:p-6 xl:p-8 bg-slate-50/50">
