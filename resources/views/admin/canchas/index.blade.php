...
@section('content')
    ...
    @if($canchasAgrupadas->isNotEmpty())
        <div x-data="{ tab: '{{ array_key_first($canchasAgrupadas->toArray()) }}' }">
            ...
            <div class="flex border-b border-gray-200 mb-6 space-x-6 overflow-x-auto no-scrollbar">
            ...
            @foreach($canchasAgrupadas as $ubicacion => $canchas)
                <div x-show="tab === '{{ $ubicacion }}'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" style="display: none;">
